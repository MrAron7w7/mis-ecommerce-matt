...
import { updateProduct } from '@/actions/seller/product.seller.action';
import { UpdateProductInput, updateProductSchema } from '@/lib/schemas/seller/product.schema';
...
export default function EditProductForm({ product, categories }: Props) {
  ...
  const {
    ...
    defaultValues: {
      ...
      variants: product.variants.map((v) => ({ type: v.type, value: v.value })),
    },
  });
  ...
  }, [nameValue, product.name, setValue]);
  ...
  const onSubmit = (data: FormData) => {
    ...
    startTransition(async () => {
      ...
      setSuccessMessage('¡Producto actualizado exitosamente! Redirigiendo...');
      setTimeout(() => {
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 mt-1">Modifica los datos del producto {product.name}</p>
            </div>
          </div>
        </div>
              ...
              {fields.length === 0 ? (
                ...
                <div className="space-y-3">
                  {fields.map((field, index) => (
                    <div
                      key={field.id}
                      className="flex gap-3 items-start p-3 bg-gray-50 rounded-xl group"
                    >
